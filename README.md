...
- **Healing.** Anyone can feed a tamed phantom ordinary food. If the owner or another player holds that food nearby — anything except a poisonous potato or the release item — the phantom stops, turns toward them, and nods. A poisonous potato or the release item still makes it stop and turn, but it shakes its head from side to side; only the owner can give those, and someone else's click also puffs angry villager particles. Food held by a rider of that phantom does not count, and a phantom that already has a rider does not stop.
